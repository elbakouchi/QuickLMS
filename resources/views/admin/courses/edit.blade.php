    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('content') ? 'is-invalid' : '' }}" name="content" id="content">{!! old('content', $course->content) !!}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('free_course') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="free_course" id="free_course" value="1" {{ $course->free_course || old('free_course', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ $course->status || old('status', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('authors') ? 'is-invalid' : '' }}" name="authors[]" id="authors" multiple required>
                    @foreach($authors as $id => $author)
                        <option value="{{ $id }}" {{ (in_array($id, old('authors', [])) || $course->authors->contains($id)) ? 'selected' : '' }}>{{ $author }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('topic') ? 'is-invalid' : '' }}" name="topic_id" id="topic_id" required>
                    @foreach($topics as $id => $topic)
                        <option value="{{ $id }}" {{ ($course->topic ? $course->topic->id : old('topic_id')) == $id ? 'selected' : '' }}>{{ $topic }}</option>
                    @endforeach
                </select>
                @if($errors->has('topic'))
                    <div class="invalid-feedback">
